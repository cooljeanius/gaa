...
{
	printf("GAA version \n"VERSION);
	printf("Copyright (C) 1998 - 1999 Joran Maille <joran@writeme.com>\n\n""Copyright (C) 2002 Nikos Mavroyanopoulos <nmav@gnutls.org>\n");
	printf("\n");
	printf("Usage: gaa [options] input-file.gaa\n");
	__gaa_helpsingle('o', "output", "file ", "specifies output file");
	__gaa_helpsingle('i', "header", "file ", "specifies header file");
	__gaa_helpsingle('p', "prefix", "PFX ", "Prefixes all symbols with that prefix.");
	__gaa_helpsingle('h', "help", "", "shows this help text");
	__gaa_helpsingle('v', "version", "", "shows gaa's version");
	__gaa_helpsingle('s', "skel", "skeleton ", "specifies an alternative skeleton file");
	printf("Please report any bug to <nmav@gnutls.org>\n");
...
#define GAAOPTID_skel	1
#define GAAOPTID_version	2
#define GAAOPTID_help	3
#define GAAOPTID_prefix	4
#define GAAOPTID_header	5
#define GAAOPTID_output	6
...
struct GAAOPTION_skel 
{
	char* arg1;
	int size1;
};

struct GAAOPTION_prefix 
{
	char* arg1;
	int size1;
};

struct GAAOPTION_header 
{
	char* arg1;
	int size1;
};

struct GAAOPTION_output 
{
	char* arg1;
	int size1;
};
...
static int gaa_get_option_num(char *str, int status)
{
    switch(status)
        ...
        case GAA_LETTER_OPTION:
			GAA_CHECK1STR("s", GAAOPTID_skel);
			GAA_CHECK1STR("p", GAAOPTID_prefix);
			GAA_CHECK1STR("i", GAAOPTID_header);
			GAA_CHECK1STR("o", GAAOPTID_output);
...
#line 277 "gaa.skel"
        ...
        case GAA_WORD_OPTION:
			GAA_CHECKSTR("skel", GAAOPTID_skel);
			GAA_CHECKSTR("version", GAAOPTID_version);
			GAA_CHECKSTR("help", GAAOPTID_help);
			GAA_CHECKSTR("prefix", GAAOPTID_prefix);
			GAA_CHECKSTR("header", GAAOPTID_header);
			GAA_CHECKSTR("output", GAAOPTID_output);
...
static int gaa_try(int gaa_num, int gaa_index, gaainfo *gaaval, char *opt_list)
{
	...
	struct GAAOPTION_skel GAATMP_skel;
	struct GAAOPTION_prefix GAATMP_prefix;
	struct GAAOPTION_header GAATMP_header;
	struct GAAOPTION_output GAATMP_output;
...
#line 393 "gaa.skel"
	...
	OK = 0;
		GAA_TESTMOREARGS;
		GAA_FILL(GAATMP_skel.arg1, gaa_getstr, GAATMP_skel.size1);
		gaa_index++;
#line 40 "gaa.gaa"
{ gaaval->skel = GAATMP_skel.arg1 ;};
...
{ gaa_help(); exit(0);  ;};
		...
		return GAA_OK;
		break;
	case GAAOPTID_prefix:
	OK = 0;
		GAA_TESTMOREARGS;
		GAA_FILL(GAATMP_prefix.arg1, gaa_getstr, GAATMP_prefix.size1);
		gaa_index++;
#line 37 "gaa.gaa"
{ gaaval->prefix = GAATMP_prefix.arg1 ;};

		return GAA_OK;
		break;
	case GAAOPTID_header:
	OK = 0;
		GAA_TESTMOREARGS;
		GAA_FILL(GAATMP_header.arg1, gaa_getstr, GAATMP_header.size1);
		gaa_index++;
#line 36 "gaa.gaa"
{ gaaval->header = GAATMP_header.arg1 ;};

		return GAA_OK;
		break;
	case GAAOPTID_output:
	OK = 0;
		GAA_TESTMOREARGS;
		GAA_FILL(GAATMP_output.arg1, gaa_getstr, GAATMP_output.size1);
		gaa_index++;
#line 35 "gaa.gaa"
{ gaaval->output = GAATMP_output.arg1 ;};
...
#line 21 "gaa.gaa"
{ gaaval->input = NULL; gaaval->output = NULL; gaaval->header = NULL; gaaval->prefix = NULL;
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(WINDOWS)
        gaaval->skel = NULL;
#else
	gaaval->skel = SKEL_DIR "/gaa.skel";
#endif 
gaaval->win32 = 0; ;};
